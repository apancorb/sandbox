...
export const collectAndRemove = async (
    s: Staker
): Promise<void> => {
    await remove(s, true);
    s.position.liquidity = 
        await getPositionLiquidity(s);
    await remove(s);
    s.metrics.summary = 
        summaryMetrics(s);
    s.log.info(
...
export const remove = async (
    ...
    for (let i: number = 0; i < s.config.tries; i++) {
        try {
            const data: MethodParameters = 
                await removeData(s, collect);
            const txr: TransactionReceipt = 
                await removeTx(s, data);
            const metrics: RemoveMetrics =
                await removeMetrics(s, txr, i, start);
            let obj: any = { 
                meta: s.metrics.meta,
                position: s.position
            };
...
export const removeData = async (
    ...
    const collectOptions: CollectOptions = {
        ...
        expectedCurrencyOwed0: 
            CurrencyAmount.fromRawAmount(s.pool.token0, 0),
        expectedCurrencyOwed1: 
            CurrencyAmount.fromRawAmount(s.pool.token1, 0),
        recipient: 
            collect ?
            s.net.recipient :
            s.net.address
    ...
    const removeLiquidityOptions: RemoveLiquidityOptions = {
        ...
        liquidityPercentage: 
            collect ? 
            new Percent(1, 1000000000) :
            new Percent(1),
        slippageTolerance: 
            new Percent(s.config.slippageTolerance, 100),
        deadline: 
            Date.now() + 1800,
        collectOptions: 
            collectOptions
...
const removeMetrics = async (
    ...
    const data: string = 
        txr.logs[txr.logs.length - 2].data;
    ...
    const removed0$: number = Number.parseFloat(
        (price0$ * removed0).toFixed(2)
    );
    const removed1$: number = Number.parseFloat(
        (price1$ * removed1).toFixed(2)
    );
...
const summaryMetrics = (
    ...
    const collected$: number = (
        s.metrics.collect.removed$
    );
    const impermanent$: number = Number.parseFloat(
        (
            s.metrics.remove.removed$ 
            - s.metrics.mint.staked$
        ).toFixed(2)
    );
    const gas$: number = Number.parseFloat(
        (
    ...
    const net$: number = Number.parseFloat(
        (
            collected$ + impermanent$ - gas$
        ).toFixed(2)
    );
    const time: string = msToHMS(
        s.metrics.remove.end.getTime()
        - s.metrics.mint.start.getTime()
    );
    return {
        collected$: collected$,
...
const getPositionLiquidity = async (
    s: Staker
): Promise<string> => {
    const manager: Contract = 
        getPositionManagerContract(s.net.provider);
    const slot: any[] = 
        await manager.positions(s.position.tokenId);
