...
import { CurrencyAmount, Fraction, Token, Percent } from "@uniswap/sdk-core";
import {
  AlphaRouter,
  SwapAndAddConfig,
  SwapAndAddOptions,
...
import { AddLiquidityOptions, Pool, Position } from "@uniswap/v3-sdk";

import { MintMetrics, Position as PositionData, Staker } from "../stake";
import {
...
export const mint = async (
  ...
  for (let i: number = 0; i < s.config.tries; i++) {
    try {
      const data: SwapToRatioRoute = await mintData(s);
      const txr: TransactionReceipt = await mintTx(s, data);
      s.position = await positionData(s, txr);
      s.metrics.mint = await mintMetrics(s, txr, i, start);
      s.log.info(
        JSON.stringify({
          meta: s.metrics.meta,
          position: s.position,
      ...
      s.log.error(JSON.stringify(getError("mint", e as Error)));
...
const mintData = async (s: Staker): Promise<SwapToRatioRoute> => {
  const router = new AlphaRouter({
    chainId: s.net.chainId,
    provider: s.net.provider,
  });
  const token0Balance: CurrencyAmount<Token> = CurrencyAmount.fromRawAmount(
    s.pool.token0,
    (await balanceOf(s.net.provider, s.pool.token0.address, s.net.address))
      .sub(10000)
      .toString(),
  );
  const token1Balance: CurrencyAmount<Token> = CurrencyAmount.fromRawAmount(
    s.pool.token1,
    (await balanceOf(s.net.provider, s.pool.token1.address, s.net.address))
      .sub(10000)
      .toString(),
  );
  ...
  const swapAndAddConfig: SwapAndAddConfig = {
    maxIterations: 10,
    ratioErrorTolerance: new Fraction(s.config.slippageTolerance, 100),
  };
...
const positionData = async (
  ...
  const log: Log = txr.logs.find((log) => log.topics.length === 2);
  const tokenId: number = BigNumber.from(log.topics[1]).toNumber();
  const manager: Contract = getPositionManagerContract(s.net.provider);
  const slot: any[] = await manager.positions(tokenId);
  const tickLower: number = BigNumber.from(slot[5]).toNumber();
  const tickUpper: number = BigNumber.from(slot[6]).toNumber();
  const liquidity: string = BigNumber.from(slot[7]).toString();
  const url: string =
    `https://app.uniswap.org/#/pool/` + `${tokenId}` + `?chain=polygon`;
  return {
    tokenId: tokenId,
    tickLower: tickLower,
    tickUpper: tickUpper,
...
const mintMetrics = async (
  ...
  const staked0$: number = Number.parseFloat((price0$ * staked0).toFixed(2));
  const staked1$: number = Number.parseFloat((price1$ * staked1).toFixed(2));
