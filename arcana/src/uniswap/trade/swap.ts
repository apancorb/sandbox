...
export const swap = async (
  ...
  for (let i: number = 0; i < t.config.tries; i++) {
    try {
      const route: SwapRoute = await swapData(t, tokenIn, tokenOut);
      const txr: TransactionReceipt = await swapTx(t, route);
      const metrics: SwapMetrics = await swapMetrics(
        t,
        type,
        tokenIn,
        tokenOut,
      ...
      t.log ? t.log.info("Swap:", e) : console.log("Swap:", e);
    }
  }
  // swap failed
...
const swapData = async (
  ...
  const amount: CurrencyAmount<Token> = CurrencyAmount.fromRawAmount(
    tokenIn,
    (
      await balanceOf(t.net.provider, tokenIn.address, t.net.address)
    ).toString(),
  );
  if (amount.equalTo(0)) {
  ...
  const swapConfig: SwapOptions = {
    recipient: t.net.recipient ? t.net.recipient : t.net.address,
    slippageTolerance: new Percent(t.config.slippageTolerance, 100),
...
const swapMetrics = async (
  ...
  const amountIn: number = Number.parseFloat(
    CurrencyAmount.fromRawAmount(
      tokenIn,
      BigNumber.from(amountInData).toString(),
    ).toFixed(tokenIn.decimals),
  );
  const amountOut: number = Number.parseFloat(
    CurrencyAmount.fromRawAmount(
      tokenOut,
      BigNumber.from(amountOutData).toString(),
  ...
  const tick: number = Number.parseInt(
    BigNumber.from("0x" + swapData.substring(2 + 4 * 64)).toString(),
  );
  console.log(amountIn, amountOut, sqrtPriceX96, liquidity, tick);
  const price$: number = Number.parseFloat(
    new Pool(tokenIn, tokenOut, 500, sqrtPriceX96, liquidity, tick)
      .priceOf(t.target)
      .toFixed(2),
  );
  const amountIn$: number = Number.parseFloat(
  ...
  const gasUsed$: number = Number.parseFloat(
    ((await getTokenPrice(t.net.provider, t.net.coin)) * gasUsed).toFixed(2),
  );
...
const swapSummary = (
  ...
  const price0$: number = t.metrics.buy.price$;
  const price1$: number = sell.price$;
  ...
  const totalTime: string = msToHMS(
    sell.end.getTime() - t.metrics.buy.start.getTime(),
  );
